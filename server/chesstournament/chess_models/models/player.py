...
class Player(models.Model):
    ...
    lichess_username = models.CharField(max_length=255, unique=True,
    ...
    def fetch_lichess_data(self):
        if self.lichess_username:
            ...
            if response.status_code == 200:
                ...
                self.lichess_rating_bullet = perfs.get("bullet", {})\
                    .get("rating", 0)
                self.lichess_rating_blitz = perfs.get("blitz", {})\
                    .get("rating", 0)
                self.lichess_rating_rapid = perfs.get("rapid", {})\
                    .get("rating", 0)
                self.lichess_rating_classical = perfs.get("classical", {})\
                    .get("rating", 0)
            elif response.status_code == 404:
                raise ValidationError(f"El usuario {self.lichess_username}\
                                      no existe en Lichess.")
            else:
                raise ValidationError(f"Error al obtener datos de Lichess\
                                      para {self.lichess_username}:\
    ...
    def get_lichess_user_ratings(self):
        if self.lichess_username:
            ...
            if response.status_code == 200:
                ...
                raise LichessAPIError(f"El usuario {self.lichess_username}\
                                      no existe en Lichess.")
            else:
                raise LichessAPIError(f"Error al obtener datos de Lichess\
                                      para {self.lichess_username}:\
                                        {response.status_code}")

    def save(self, *args, **kwargs):
        existing_player = Player.objects.filter(
            models.Q(name=self.name, name__isnull=False, email=self.email,
                     email__isnull=False) |
            models.Q(lichess_username=self.lichess_username,
                     lichess_username__isnull=False) |
        ...
        if existing_player:
            for field in self._meta.fields:
                if getattr(self, field.name) is not None:
                    setattr(existing_player, field.name,
